...
% Course     : DSP Lab 2021
% 
% DetaiLs    : This fiLe calculates BER performance for BPSK/QPSK signal
...
parallel_si_qpsk=ifft(parallel_qpsk,fftSize);
parallel_si_qpsk_cp = vertcat(parallel_si_qpsk(N-Lcp+1:N,:),parallel_si_qpsk);
si_qpsk_cp = reshape(parallel_si_qpsk_cp,[],1);
%% AWGN noise
...
for snr=0:5:30
    ...
    for j= 1:N
        ...
        wireless_out_bpsk = horzcat(wireless_out_bpsk,cconv(h,parallel_si_bpsk_cp(:,j),fftSize + Lcp));
        wireless_out_qpsk = horzcat(wireless_out_qpsk,cconv(h,parallel_si_qpsk_cp(:,j),fftSize + Lcp));
    end
    wireless_out_bpsk = reshape(wireless_out_bpsk,[],1);
    wireless_out_qpsk = reshape(wireless_out_qpsk,[],1);
    %%
    
    n_bpsk=sigma_bpsk(i+1)*(randn(M_bpsk,1)+1i*randn(M_bpsk,1));
    n_qpsk=sigma_qpsk(i+1)*(randn(M_qpsk,1)+1i*randn(M_qpsk,1));
    
    r_bpsk_cp=wireless_out_bpsk + n_bpsk;
    r_qpsk_cp=wireless_out_qpsk + n_qpsk;
    
%% OFDM Receiver
    parallel_r_bpsk_cp=reshape(r_bpsk_cp,[fftSize + Lcp, N]);
    parallel_r_qpsk_cp=reshape(r_qpsk_cp,[fftSize + Lcp, N]);
    
    parallel_r_bpsk = parallel_r_bpsk_cp(Lcp+1:fftSize + Lcp,:);
    parallel_r_qpsk = parallel_r_qpsk_cp(Lcp+1:fftSize + Lcp,:);
    
    parallel_X_bpsk=fft(parallel_r_bpsk,fftSize)./H;
    parallel_X_qpsk=fft(parallel_r_qpsk,fftSize)./H;
