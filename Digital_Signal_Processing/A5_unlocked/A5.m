...
for i=0:50
    ...
    yn=parallel_si_qpsk+n_qpsk;
    yn=reshape(yn,1,totalSubcarr);
    ...
    %estimate_no(i+1)=abs(sum((fftSize*log((sum(conj(Hk(2:fftSize,:)).*Hk(1:fftSize-1,:))/(fftSize-1))))/1i*2*pi)/(totalSubcarr/fftSize));
end
hold on
grid on
plot(SNRb,mse);
